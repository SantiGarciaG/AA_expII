...
class AvoidanceExp:    
    ...
    def run_exp(self, test_mode=False):
        ...
        # assume participants start with 'avoid'
        approach_preferred = False
        while not approach_preferred:
#        for i in range(1, N_BLOCKS+1):
            # Taking approach-deck as reference, 
            # REMEMBER: rewards[0] is reward for approach/threat; rewards[1] is avoid/neutral card         
            p_approach = self.run_block(i, rewards, MIN_N_TRIALS, is_club_left=is_club_left, 
                                   is_baseline=False)  
    ...
    def run_block(self, block_number, rewards, min_n_trials, is_club_left=True, is_baseline=False):
